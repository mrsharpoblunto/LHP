...
std::ostream& operator<<(std::ostream& stream, const LHP::Json::Value& v) {
    if (v.is<double>()) {
        return stream << v.get<double>();
    } else if (v.is<std::string>()) {
        return LHP::Json::stream_string(stream, v.get<std::string>());
    } else if (v.is<bool>()) {
        if (v.get<bool>()) {
            return stream << "true";
        } else {
    ...
    } else if (v.is<LHP::Json::Value::Null>()) {
        return stream << "null";
    } else if (v.is<LHP::Json::Object>()) {
        return stream << v.get<LHP::Json::Object>();
    } else if (v.is<LHP::Json::Array>()){
        return stream << v.get<LHP::Json::Array>();
    }
    // Shouldn't reach here.
    return stream;
}

std::ostream& operator<<(std::ostream& stream, const LHP::Json::Array& v) {
    stream << "[";
    const std::vector<LHP::Json::Value*>& values(v.values());
...
std::ostream& operator<<(std::ostream& stream, const LHP::Json::Object& v) {
    stream << "{";
    const std::map<std::string, LHP::Json::Value*>& kv(v.kv_map());
    for (std::map<std::string, LHP::Json::Value*>::const_iterator i = kv.begin();
         i != kv.end(); /**/) {
        LHP::Json::stream_string(stream, i->first);
        stream << ": " << *(i->second);
        ++i;
        if ( i != kv.end()) {
            stream << ", ";
